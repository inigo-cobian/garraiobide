...
std::vector<Stop> GtfsManager::get_stops() const {
    ...
    std::vector columns = {fields::stops::ID, fields::stops::NAME, fields::stops::LATITUDE, fields::stops::LONGITUDE, fields::stops::TYPE, fields::stops::PARENT};
    auto result = io::CsvReader::parse_file(csv, ',', columns);

    std::vector<Stop> stops;
    for (auto line : result) {
        auto stop = Stop(line.at(fields::stops::ID), line.at(fields::stops::NAME), std::stof(line.at(fields::stops::LATITUDE)), std::stof(line.at(fields::stops::LONGITUDE)));
...
std::vector<Route> GtfsManager::get_routes() const {
    ...
    for (auto line : result) {
        std::string name = line.at(fields::routes::LONG_NAME).empty() ? line.at(fields::routes::SHORT_NAME) : line.at(fields::routes::LONG_NAME);
        auto route = Route(line.at(fields::routes::ID), name, line.at(fields::routes::TYPE), line.at(fields::routes::COLOR), line.at(fields::routes::TEXT_COLOR));
